  ...
  lines.each_with_index do |line, line_number|
    ...
    matches.each do |match|
      match_data = {:value => match,
                    :index => Regexp.new(match) =~ line,
                    :line => line_number}
      array << match.to_i if check_if_parts(match_data, lines)
    end
  end
  p array
end

def check_if_parts(match, lines)
  length = match[:value].size
  to_check = []

  if match[:index] >= 1
    to_check << lines[match[:line] - 1][(match[:index] - 1 )..(match[:index] + length) ] if (match[:line] - 1) >= 0
    to_check << lines[match[:line]][(match[:index] - 1 )..(match[:index] + length) ]
    to_check << lines[match[:line] + 1][(match[:index] - 1 )..(match[:index] + length) ] if lines[match[:line] + 1]
  else
    to_check << lines[match[:line] - 1][(match[:index])..(match[:index] + length) ] if (match[:line] - 1) >= 0
    to_check << lines[match[:line]][(match[:index])..(match[:index] + length) ]
    to_check << lines[match[:line] + 1][(match[:index])..(match[:index] + length) ] if lines[match[:line] + 1]
  end
  # p match
  # p to_check
  to_check.any? { |string| string.match(/[^\d.\n]/)}
end

#  =======================================

input = "....401.............425.......323......791......697...............963............................................420........................
...*..................................%......#.....*....290.........................492.............656...@953.....................+830.....
