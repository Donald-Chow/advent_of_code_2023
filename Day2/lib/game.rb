def check_possibility(input)
  # input is a text block of game
  # break down to each line
...
def parse_game(input)
  # code
  input.split("\n").map do |line|
    match = line.match(/Game (?<game>\d+): (?<results>(\d+ (blue|green|red)[,;]? ?)+)/)
    game_id = match[1].to_i
    game_results = match[2].split(";").map do |game|
      {:blue  => (game.match(/(?<blue>\d+) blue/)[0] if game.match(/(?<blue>\d+) blue/)).to_i,
       :red   => (game.match(/(?<red>\d+) red/)[0] if game.match(/(?<red>\d+) red/)).to_i,
       :green => (game.match(/(?<green>\d+) green/)[0] if game.match(/(?<green>\d+) green/)).to_i
      }
    end
    { id: game_id,  results: game_results }
  end
end

def check_and_sum(data)
  # code
  # sum if is possible 12 red cubes, 13 green cubes, and 14 blue cube
  data.inject(0) do |sum, game|
    if game[:results].all? do |results|
      (results[:green] ? results[:green] <= 13 : true) &&
      (results[:red] ? results[:red] <= 12 : true) &&
      (results[:blue] ? results[:blue] <= 14 : true)
    end
      sum + game[:id]
    else
      sum
